...
bool GUI::Initialize( std::shared_ptr<Window> window )
{
    ...
    ThrowIfFailed(D3DReadFileToBlob(FULL_SHADER_PATH(L"ImGUI_VS.cso"), &vertexShaderBlob));

    // Load the pixel shader.
    ComPtr<ID3DBlob> pixelShaderBlob;
    ThrowIfFailed(D3DReadFileToBlob(FULL_SHADER_PATH(L"ImGUI_PS.cso"), &pixelShaderBlob));
