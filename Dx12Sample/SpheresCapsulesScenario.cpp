...
void SpheresCapsulesScenario::onLoadInternal(CommandList& commandList)
{
	...
	for (int i = 0; i < m_capsulesCount; i++)
	{
		...
		obj = std::make_shared<PhysicsObject>(MeshType::Capsule, capsuleMesh.get(), stat ? GlobalData::g_staticTexture.get() : GlobalData::g_customTexture.get());
...
void SpheresCapsulesScenario::drawImGui() {
